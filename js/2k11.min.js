...
});});};})(jQuery);(function(r,p,n){var w="placeholder" in p.createElement("input"),t="placeholder" in p.createElement("textarea"),o=n.fn,u=n.valHooks,l,m;
if(w&&t){m=o.placeholder=function(){return this;};m.input=m.textarea=true;}else{m=o.placeholder=function(){return this.filter((w?"textarea":":input")+"[placeholder]").not(".placeholder").bind({"focus.placeholder":v,"blur.placeholder":s}).data("placeholder-enabled",true).trigger("blur.placeholder").end();
};m.input=w;m.textarea=t;l={get:function(a){var b=n(a);return b.data("placeholder-enabled")&&b.hasClass("placeholder")?"":a.value;},set:function(a,c){var b=n(a);
if(!b.data("placeholder-enabled")){return a.value=c;}if(c==""){a.value=c;s.call(a);}else{if(b.hasClass("placeholder")){v.call(a,true,c)||(a.value=c);}else{a.value=c;
}}return b;}};w||(u.input=l);t||(u.textarea=l);n(function(){n(p).delegate("form","submit.placeholder",function(){var a=n(".placeholder",this).each(v);setTimeout(function(){a.each(s);
},10);});});n(r).bind("beforeunload.placeholder",function(){n(".placeholder").each(function(){this.value="";});});}function q(a){var b={},c=/^jQuery\d+$/;
...
if(a===true){return c[0].value=d;}c.focus();}else{b.value="";c.removeClass("placeholder");}}}function s(){var c,b=this,d=n(b),a=d,e=this.id;if(b.value==""){if(b.type=="password"){if(!d.data("placeholder-textinput")){try{c=d.clone().attr({type:"text"});
}catch(f){c=n("<input>").attr(n.extend(q(this),{type:"text"}));}c.removeAttr("name").data({"placeholder-password":true,"placeholder-id":e}).bind("focus.placeholder",v);
d.data({"placeholder-textinput":c,"placeholder-id":e}).before(c);}d=d.removeAttr("id").hide().prev().attr("id",e).show();}d.addClass("placeholder");d[0].value=d.attr("placeholder");
}else{d.removeClass("placeholder");}}}(this,document,jQuery));jQuery(function(b){(function(){var a=this;this.hideDetailChildren=function(l){var k=l instanceof jQuery?l[0].childNodes:l.childNodes,j=k.length;
...
b("input, textarea").placeholder();b("#trackback_url>a,.short-url").click(function(){var c=b(this).attr("title");alert(c);return false;});b(".comment_reply").click(function(){var c=b(this).attr("id").replace(/serendipity_reply_/g,"");
