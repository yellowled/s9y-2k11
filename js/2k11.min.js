...
a("head").append(b);}a.each(a("details"),function(f,e){c.hideDetailChildren(e);});}})();});var AccessifyHTML5=function(b){var a={article:{role:"article"},aside:{role:"complementary"},nav:{role:"navigation"},main:{role:"main"},output:{"aria-live":"polite"},section:{role:"region"},"[required]":{"aria-required":"true"}};
if(b){if(b.header){a[b.header]={role:"banner"};}if(b.footer){a[b.footer]={role:"contentinfo"};}if(b.main){a[b.main]={role:"main"};a.main={role:""};}}jQuery.each(a,function(c,d){jQuery(c).not("["+d[0]+"]").attr(d);
});};
